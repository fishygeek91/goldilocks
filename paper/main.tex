...
pseudo-Boolean solvers (a one-to-three-order wall-clock gap to the
quench at low energy once the right solver is used, with the CNF route
worse by a further one to three orders).
...
states. The honest anchor is therefore this: the low-energy shell is
reachable de novo, and with the right classical tool it costs
$10^{-1}$--$10^{1}$\,s per shell at $n=20$ against $\sim 10^{-2}$\,s per
quench proposal on hardware --- a one-to-three-order gap whose scaling
with $n$ we have not measured with the native solver. Two facts
survive tool choice: the deep shells the quench targets are tiny at
this window, and every classical route to them has to \emph{decide}
near-empty integer windows, which no cheap kernel of
Fig.~\ref{fig:cfar} attempts at all.
...
of the earlier M03/M04 ``cold'' timings). A public archival deposit will be made at submission
(TODO: ZENODO-DOI).
