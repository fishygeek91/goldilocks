...
\author{Dustin Kovac}
\affiliation{Independent researcher, Pinedale, Wyoming, USA}
...
spread. Finally we test what the
coherent quench's value actually is, against a fourteen-kernel
exact-tier zoo: energy concentration alone does not predict advantage
(informed local kernels concentrate best and mix worst), but
concentration \emph{at Hamming distance} does, and an oracle that
merely proposes uniformly from the target's energy shell matches or
beats the quench everywhere --- a mechanism no cheap classical kernel
we construct reproduces, and whose exact classical emulation pays a
measured certification cost of at least one order of magnitude every
four spins, because an exact shell proposal must certify a near-empty
energy window complete --- a proof a physical sampler never performs.
That asymmetry generalizes: a conserved diagonal charge confines every
measured outcome to its symmetry sector while preserving evaluation-free
exactness within it, a feasibility certificate priced at zero; but a
pre-registered race of the amortized architecture this suggests ---
device-mined proposal libraries consumed by exact mode-jumping ---
is killed by its own criteria, classically. Noise costs nothing and buys nothing; the
quench's edge is global isoenergetic proposals.
\end{abstract}
...
\item \emph{The quench's mechanism, measured against its classical
competition} (Secs.~\ref{sec:outlook} and~\ref{sec:outlook-test}). The
scalar energy-concentration conjecture is refuted by a fourteen-kernel
exact-tier zoo; concentration \emph{at Hamming distance} survives as the
predictor; an oracle that merely proposes uniformly from the energy
shell matches or beats the quench everywhere it is connected; and three
classical routes to that shell are measured and fall short ---
informed local kernels (monotonically in their informedness),
two-replica isoenergetic cluster moves (never above the
mixture-dilution floor), and de novo shell sampling by SAT and
pseudo-Boolean solvers (a wall-clock gap to the quench that grows from
about three orders at $n=16$--$20$ to at least four and a half by
$n=24$, because an exact proposal must certify a near-empty energy
window complete).
...
\item \emph{Extraction pricing as a design rule, with a free-certificate
theorem and a registered negative} (Secs.~\ref{sec:exactness-sectors}
and~\ref{sec:extraction}). Devices charge per measured shot
($10^{2}$--$10^{4}$\,s$^{-1}$) while classical samplers execute
$10^{6}$--$10^{8}$ proposals per second, so any hybrid that consumes
device output per MCMC step starts $10^{4}$--$10^{6}\times$ behind on
targets whose $\pi$-evaluation is cheap relative to shot latency. What
noisy hardware does give away is invariants: a conserved diagonal
charge confines every outcome to its symmetry sector (a feasibility
certificate at zero cost, Theorem~\ref{thm:sector}) while preserving
the evaluation-free symmetry of Sec.~\ref{sec:exactness} within each
sector. We tested the one architecture that takes the device off the
per-step path --- a fixed atom library mined offline, consumed by exact
mode-jumping proposals --- under pre-registered kill criteria (run
P01), and both kills fired: classical miners matched the simulated
quench's mode coverage in every cell at matched budget, and the darting
gain never reached $2\times$ over parallel tempering on
metastability-screened targets at $n=50$--$200$. The negative is
...
delocalization. Section~\ref{sec:outlook} states the forward-looking
predictor, energy concentration, as a conjecture --- and then tests it
on a fourteen-kernel exact-tier zoo (Sec.~\ref{sec:outlook-test}): the
scalar fails, concentration \emph{at distance} holds, and a shell
oracle shows the mechanism is sufficient. Related work is collected in Sec.~\ref{sec:related} rather
...
\subsection{Free certificates from conserved charges}
\label{sec:exactness-sectors}

The exactness results above have a feasibility-side companion. Let
$q:\{0,1\}^n\to\mathbb{R}$ be a classical constraint function and
$Q=\sum_x q(x)|x\rangle\!\langle x|$ the corresponding diagonal charge, so
that each $Q$-eigenspace is spanned by computational-basis states.

\begin{theorem}[Sector support]\label{thm:sector}
Prepare $|x\rangle$, apply any finite composition of unitaries $U$ with
$[U,Q]=0$ and CPTP maps whose every Kraus operator commutes with $Q$,
and measure in the computational basis. Then every outcome $y$
satisfies $q(y)=q(x)$ with probability 1, at any depth, evolution time,
and system size. If additionally the hypotheses of
Theorem~\ref{thm:lindblad} hold (real symmetric $H$, transpose-closed
jumps), the proposal is symmetric \emph{within every sector
simultaneously}, hence a valid $\log q\equiv 0$ MH proposal for any
target restricted to the start sector.
\end{theorem}

The proof is commutant algebra plus the time-reversed unravelling of
Theorem~\ref{thm:lindblad}, sector-restricted; commuting operators are
block-diagonal across $Q$-eigenspaces, so sector membership is
preserved by induction over the composition, and the symmetry argument
is unchanged (proof and a machine certification --- $n=6$ number-conserving
XY dynamics with diagonal disorder under full-Lindblad dephasing;
cross-sector leakage exactly zero, symmetry defect
$1.9\times 10^{-16}$ --- are distributed with the manuscript source).
Two readings matter here. First, every diagonal jump operator commutes
with \emph{every} diagonal $Q$, so computational-basis dephasing ---
the dominant channel, and the free one by Theorem~\ref{thm:lindblad}
--- is automatically sector-preserving: for diagonal charges, dephasing
costs neither exactness nor feasibility. Second, the theorem prices at
zero exactly what Sec.~\ref{sec:outlook-test} measures as the scaling
cost of every classical route to the energy shell: a classical exact
proposal must \emph{certify} its constraint set (the completeness
proofs censored at $n\ge 24$), while conserving dynamics cannot leave
it. The ingredients are individually standard --- decoherence-free
subspaces~\cite{Lidar1998}, symmetry-verification error
mitigation~\cite{BonetMonroig2018}, conserving drivers for constrained
annealing and QAOA~\cite{HenSpedalieri2016,Hadfield2019} --- but they
have been used to protect or mitigate an output-priced computation;
here the sector \emph{is} the encoding, for sampling, inside an exact
MH boundary. Hamming-weight sectors are the natural hardware
instantiation (XY/iSWAP gate sets conserve particle number exactly).
What the theorem does not give: any statement under amplitude damping.
Discarding sector-violating shots (a classical $O(n^2)$ check per
shot) restores the support guarantee by fiat, and we conjecture it
also preserves proposal symmetry --- but the discard probability is
state-dependent for exactly the reason $T_1$ breaks
Theorem~\ref{thm:tc}, so this remains a conjecture, not a corollary,
and we do not use it. Fair sampling \emph{within} a sector is likewise
not implied (annealers are measurably unfair samplers of degenerate
sets~\cite{Konz2019}); it is an empirical question, and
Sec.~\ref{sec:extraction} is what measuring it honestly looks like.
...
Nothing in this section is yet a design rule for future kernels. The
...
targets is stated as a conjecture in Sec.~\ref{sec:outlook} and then
put to an exact-tier test in Sec.~\ref{sec:outlook-test}, where the
scalar form fails and a concentration-at-distance refinement survives.
...
\subsection{Extraction pricing, and an amortized architecture killed by
its own registration (run P01)}
\label{sec:extraction}

The two-clocks rule has a hardware-side sharpening. Present devices
deliver $10^{2}$--$10^{4}$ useful shots per second; a classical sampler
executes $10^{6}$--$10^{8}$ proposals per second. A hybrid that
consumes one device shot per MCMC step therefore pays a
$10^{4}$--$10^{6}\times$ throughput tax before any per-proposal quality
enters, while the measured per-proposal edge of the best quantum kernel
in our zoo is $1.8$--$26\times$ (Sec.~\ref{sec:outlook-test}). On any
target whose $\pi$-evaluation is cheap relative to shot latency
($\sim\!10^{-2}$\,s) --- the scope matters: an expensive-$\pi$ target
would make per-evaluation and wall-clock currency coincide --- no
per-step-device hybrid can show an honest ESS-per-second advantage
until shot rates improve by roughly three orders of magnitude. This is
the general I/O argument of Hoefler \emph{et
al.}~\cite{Hoefler2023} applied as a design selection rule for
sampling, and recent per-step proposals~\cite{AraiKadowaki2025,Lin2025}
report spectral-gap or per-step gains with no wall-clock metric ---
precisely the accounting the protocol above forbids. We suggest
benchmarks report, alongside the two clocks, the extraction cost
$B_{\mathrm{ext}} = \text{device shots consumed}/\text{ESS}$.

One architecture escapes the tax structurally: pay the device
\emph{once}, offline, to mine a fixed library of low-energy ``atom''
states, then run a purely classical exact chain whose mixture kernel
darts into the frozen library (Tjelmeland--Hegstad mode
jumping~\cite{Tjelmeland2001}; precomputed-library darting is classical
prior art in continuous spaces~\cite{Andricioaei2001,Sminchisescu2011}
and per-step-optimization variants exist for discrete
ones~\cite{Hubin2018}). The device cost amortizes; $B_{\mathrm{ext}}$
falls with chain length. We raced this architecture under
pre-registration (run P01, protocol and data with the manuscript
source), with two kill criteria fixed before any run: K1, classical
miners (solver member-enumeration without completeness proofs,
parallel-tempering restarts, greedy descent with kicks) match the
simulated quench miner's mode coverage at matched builder wall-clock
--- quantum arm dead; K2, darting gain below $2\times$ over a
parallel-tempering baseline on targets pre-screened for measured
classical metastability --- architecture dead on this class. A
portal-acceptance gate (energy-matched atoms must accept at
$\gtrsim 0.2$) passed at $n\le 20$, so the race ran.

Both kills fired. At $n\le 20$ against enumerated truth, the best
classical miner matched or beat the quench miner's mass-weighted mode
coverage in $24$ of $24$ cells at $n\ge 16$ --- with the quench arm
overrunning its builder budget $3$--$50\times$ (one statevector
evolution at $n=20$ costs $30$--$100$\,s). At $n=50$--$200$, six of
twelve cells survived the metastability screen (the others mix under
plain single-flip MH --- racing there would manufacture a trivial K2
failure), and on the survivors the darting gain was $0.46$--$1.75\times$,
never $2\times$, with two measured mechanisms: solver member-finding
collapses from hundreds of atoms to $1$--$3$ per 30-second budget on
dense instances at $n\ge 50$ (bounding the scale reach of the
member-finding speed reported in Sec.~\ref{sec:outlook-test}), and
portal acceptance falls to $\lesssim 0.01$ because energy matching at
$\delta E \lesssim T$ does not transfer upward in $n$. Whether quantum
hardware would out-mine classical miners at $n\ge 50$ is not decidable
in simulation (statevector caps near $n=28$; tensor-network truncation
corrupts the shell physics being measured) and we make no claim either
way. The architecture that structurally escapes the extraction tax
therefore fails on dense spin glasses for classical reasons ---
which is the benchmark working as designed: the registered negative,
its screen, and $B_{\mathrm{ext}}$ are reusable as protocol pieces,
and offline-batch device reuse remains live only where the consumer
is an estimator rather than a chain~\cite{Camino2025}.

\section{Energy concentration as a predictor: conjecture, test, refinement}
\label{sec:outlook}
...
\subsection{The test, executed: the scalar fails; a refined predictor
holds}
\label{sec:outlook-test}

That test has now been run, on a kernel zoo wide enough to break the
conjecture where it is breakable (run M01; data with the manuscript
source). For fourteen exact-tier kernels --- the coherent quench;
single-flip, add--delete--swap, and uniform-flip; a Creutz-demon
microcanonical kernel~\cite{Creutz1983} at two demon temperatures;
discrete Langevin proposals~\cite{Zhang2022DLP} at three
informedness levels, including the uninformed random-mask limit; two
``soft-spin'' variants that flip weak-local-field sites preferentially;
and an oracle \emph{shell kernel}, uniform on
$S_\varepsilon(x)=\{y\neq x: |E(y)-E(x)|\le\varepsilon\}$ with the
$|S_\varepsilon|$ Hastings correction, at three widths --- we computed
$C(q)$ ($\kappa=2$) and the exact MH gap on six all-to-all spin-glass
instances with random fields ($n=8,10$; seeds 1--3) at
$T\in\{1,0.3,0.1\}$: 18 cells, 252 kernel evaluations.

The scalar conjecture fails. Across the eleven implementable kernels
the per-cell Spearman correlation between $C(q)$ and the gap has median
$\rho_s=-0.07$ (range $-0.70$ to $+0.97$ across the 18 cells). The
counterexamples are systematic, not noise: informed local kernels ---
discrete Langevin at full informedness, soft-spin at strong bias, the
cold demon --- post the \emph{highest} energy concentrations in a cell
while sitting up to ten orders of magnitude below the quench in gap.
They concentrate energy the cheap way, by proposing single-site or
downhill moves, and the same information that concentrates them makes
them greedy and local. Figure~\ref{fig:cfar}(a) is that scatter.

Two amendments repair the predictor, and both are dictated by the data
rather than fitted to it. First, the window that matters is set by the
coupling scale, not the temperature: quench and shell-oracle gaps are
$T$-independent from $T=1$ to $T=0.1$ while a $\kappa T$ window empties.
Second, concentration must be achieved \emph{at distance}. Define
\begin{equation}
  C_{\mathrm{far}}(q)
  =
  \sum_{d_H(x,y)\ge 3} \pi(x)\, q(y|x)\,
  \mathbf{1}\bigl[\,|E(y)-E(x)| \le \varepsilon\,\bigr],
  \label{eq:cfar}
\end{equation}
with $\varepsilon$ of order the coupling scale ($\varepsilon=2$ here,
$J_{ij}\sim\mathcal N(0,1)$). The per-cell correlation jumps to median
$\rho_s=+0.84$ (minimum $+0.26$), and the quench is the top-ranked
implementable kernel by $C_{\mathrm{far}}$ in 16 of 18 cells (mean rank
1.17) --- exactly where it is the top-ranked kernel by gap
[Fig.~\ref{fig:cfar}(b)].

The oracle closes the loop from correlation to mechanism. A kernel with
\emph{only} the shell property --- uniform on $S_\varepsilon(x)$,
knowing nothing about the walk, the mixer, or coherence --- matches or
beats the quench gap on every instance at $\varepsilon\approx 2$--$4$
(e.g.\ $n=10$, seed 3, $T=0.1$: shell $0.19$ vs.\ quench $0.045$; $n=8$,
seed 1: $0.15$--$0.19$ vs.\ $0.085$--$0.090$), with the one caveat that
$\varepsilon\lesssim 1$ can disconnect the state space (gap $0$;
connectivity is a separate necessary condition the scalar does not see).
The quench's value on these targets is that it \emph{is} an approximate
global shell sampler.

The qualitative half of this identification has antecedents in the
qe-MCMC literature, and we flag them explicitly. Ferguson and
Wallden~\cite{Ferguson2025} state, as the design heuristic behind
their coarse-graining, that proposals which maximize Hamming distance
while minimizing energy difference are the ones likely to be accepted,
and engineer their scheme to retain both properties. Arai and
Kadowaki~\cite{AraiKadowaki2025} measure exactly this signature ---
large $d_H$ jointly with small $|\Delta E|$ --- for quantum-annealing
proposals on spin glasses and identify it as the acceptance source.
Cao \emph{et al.}~\cite{Cao2026} note in passing that the quench
induces long-range transitions between near-degenerate states. What
those works assert or observe qualitatively, the zoo makes
quantitative and falsifiable: $C_{\mathrm{far}}$ is a measured
predictor with a refuted scalar rival, and the oracle shows the shell
property alone --- no interference, no mixer, no coherence ---
suffices, which none of the antecedents constructs or tests.

What no kernel in the zoo achieves is the conjunction cheaply. Local
information buys concentration only at $d_H=1$, and adding more of it
(discrete Langevin informedness, soft-spin bias, demon budget) moves
kernels monotonically \emph{away} from the quench corner of
Fig.~\ref{fig:cfar}(b). Distance without information (uniform,
random-mask) forfeits concentration. Selecting shell candidates from
random masks by multiple-try Metropolis~\cite{Liu2000MTM} matches the
quench per accepted step in sampled runs but spends $2K-1=15$ target
evaluations per step doing it.

The classical literature approaches the conjunction from two
directions outside our zoo, and neither closes it. Houdayer's
isoenergetic cluster moves~\cite{Houdayer2001}, extended to arbitrary
dimension by Zhu, Ochoa, and Katzgraber~\cite{Zhu2015ICM}, are
$O(n)$, rejection-free, and global in Hamming distance --- but they
conserve the joint energy of a replica \emph{pair} rather than the
energy of the updated chain, propose only within the instantaneous
two-replica overlap structure rather than uniformly on a shell, and
degenerate as clusters percolate on dense frustrated graphs such as
these. We have measured all three failure modes on the M01 instances
(run M02, data with the manuscript source): on these all-to-all
targets the faithful cluster move is exactly the replica swap --- a
statistical no-op --- restricted clusters forfeit pair-energy
conservation and post zero pair-Metropolis acceptance at $T=0.1$, the
two-replica disagreement set is empty in $52$--$100\%$ of stationary
draws at $T=0.1$, and mixing cluster moves into single-flip at equal
weight never lifts the exact two-replica gap above the
mixture-dilution floor ($0.50$--$0.51\times$ the single-flip pair
baseline in every resolvable cell). The equi-energy sampler of Kou, Zhou, and
Wong~\cite{Kou2006EE} jumps within energy rings at arbitrary Hamming
distance --- statistics has wanted this oracle for two decades --- but
draws its jump targets from stored past states of auxiliary
higher-temperature chains, so shell samples are amortized from prior
mixing, not generated fresh. Simulating the quench itself with
matrix-product states, as Christmann \emph{et
al.}~\cite{Christmann2024} propose, reaches the shell at polynomial
but distinctly superlinear cost per proposal, with the reported
advantage surviving even unconverged bond dimension.

Whether a classical kernel can sample
the energy shell at distance \emph{de novo} with $O(n)$--$O(n^2)$
work per proposal is,
after this test, the sharpest open question this program leaves; at
$n\le 10$ nothing we constructed does, and the quench's per-proposal
edge over the best implementable classical kernel in each cell runs
from $1.8\times$ to $26\times$ in gap (median $11\times$), and the
quench is the top-ranked kernel by gap in all 18 cells. (Per wall-clock second on present hardware that edge
is inverted by four orders of magnitude of shot cost; the point here is
mechanism, not deployment.)

The oracle is nonetheless NP-cheap, and we have measured what that
costs --- with one instructive false start. Encoding the shell as
integer pseudo-Boolean constraints and sampling it with random parity
constraints, in the style of hashing-based near-uniform SAT
sampling~\cite{Gomes2006XOR,Ermon2012UAI,Chakraborty2014UniGen}, yields
\emph{de novo} shell samples that agree exactly with enumeration on the
integerized Hamiltonian and are empirically uniform (total variation
$\le 0.03$ from uniform on every enumerable instance; run M03, data
with the manuscript source). Along a temperature ladder at $n=20$ (run
M05) the cost of this CNF-based route rises steeply with depth: shells
around states equilibrated above $T_c$ ($\sim\!10^{5}$ members) sample
in $\sim 10^{-1}$\,s, while below $T_c$ the shells shrink to
$10^{3}$--$10^{0}$ members and CryptoMiniSat, UniGen, and ApproxMC all
need $10^{2}$--$10^{3}$\,s or time out. That wall is largely an artifact
of the CNF translation: a pseudo-Boolean solver that takes the energy
window natively (Exact) enumerates the same deep shells in
$10^{-1}$--$10^{1}$\,s, $10$--$3{,}500\times$ faster on identical
states. Finding members is not the expensive part; an \emph{exact} proposal
must also certify that no further member exists (for the
$|S_\varepsilon(x)|/|S_\varepsilon(y)|$ Hastings ratio, or to draw
uniformly), and that completeness proof is what scales. (Member-finding
being cheap is itself a bounded claim: it is measured here at
$n\le 32$, and run P01 finds it fails by $n=50$ on dense instances at
this window --- Sec.~\ref{sec:extraction}.) With the native
solver and correctly equilibrated low-energy states (run M06), an
exactly uniform deep-shell proposal costs $1$--$4$\,s at $n=16$,
$30$--$50$\,s at $n=20$, and is censored at our $300$\,s per-call
timeout in eleven of twelve deep cells at $n=24$--$32$ --- growth of at
least an order of magnitude per four spins --- against $\sim 10^{-2}$\,s
per quench proposal on hardware, flat in $n$ at these sizes. The gap is
therefore three and a half orders at $n=20$ and at least four and a
half, widening, from $n=24$. Two facts survive tool choice: the deep
shells the quench targets are tiny at this window, and every classical
route to them has to \emph{decide} near-empty integer windows --- a
completeness proof a physical sampler never has to produce, and one that
no cheap kernel of Fig.~\ref{fig:cfar} attempts at all.

\noindent\textbf{Refined conjecture (concentration at distance).}
Advantage over trivial kernels is predicted by
$C_{\mathrm{far}}(q)$ --- proposal mass that is simultaneously
near-degenerate at the coupling scale and long-range --- subject to
shell connectivity. The scalar $C(q)$ alone is refuted as a design
objective by the informed-local counterexamples above.

\begin{figure*}
\includegraphics[width=\linewidth]{figures/fig6_concentration_at_distance.pdf}
\caption{\label{fig:cfar}
The Sec.~\ref{sec:outlook} test, executed (M01: 14 kernels $\times$ 6
spin-glass instances, $n=8,10$ $\times$ $T\in\{1,0.3,0.1\}$; exact MH
spectral gap throughout; gaps and concentrations clipped at
$10^{-12}$ and $10^{-4}$ for display).
(a)~The C8 scalar: energy concentration $C(q)$ with window $2T$ against
the exact gap, implementable kernels only. Median per-cell Spearman
$\rho_s=-0.07$: concentration alone does not predict advantage, and the
informed local kernels (triangles) occupy the high-$C$, low-gap corner.
(b)~Concentration at distance [Eq.~\eqref{eq:cfar}]: median per-cell
$\rho_s=+0.84$, the quench (stars) is top-ranked by $C_{\mathrm{far}}$
in 16 of 18 cells, and the shell oracle (open diamonds) --- uniform on
the energy shell, no quantum structure at all --- matches or beats the
quench everywhere it is connected. Bottom-right diamonds are
disconnected small-$\varepsilon$ shells: connectivity is a separate
necessary condition.}
\end{figure*}
...
baseline. Their companion analysis bounds the chain's gap by the
inverse participation ratio of classical states in the quench
eigenbasis and concludes that useful quenches must be
fine-tuned~\cite{OrfiSels2024b}, a structural barrier that our
$C_{\mathrm{far}}$ predictor renders operational. The envelope law is
complementary: per-cell, pre-registered, quantitative, on typical
Ising/SK/RFIM instances, against the full baseline envelope, with
$\gamma$-resolved erosion. We cite them as a sibling, not as a special
case of us or the reverse.
...
On the mechanism side the same line contains the qualitative
antecedents of Sec.~\ref{sec:outlook-test}, discussed there: the
coarse-graining of Ref.~\cite{Ferguson2025} is engineered around
large-$d_H$, small-$|\Delta E|$ proposals; Arai and
Kadowaki~\cite{AraiKadowaki2025} observe that signature in
quantum-annealing proposals; Cao \emph{et al.}~\cite{Cao2026} note
long-range near-degenerate transitions while pursuing irreversible
variants. Relative to all three, what is new in
Sec.~\ref{sec:outlook-test} is the oracle-sufficiency test, the
quantitative predictor with its refuted rival, and the classical
no-go evidence.
...
A fourth neighborhood enters through
Sec.~\ref{sec:outlook-test}: informed proposals for discrete
distributions. Locally balanced proposals~\cite{Zanella2020},
Gibbs-with-gradients~\cite{Grathwohl2021}, and discrete Langevin
proposals~\cite{Zhang2022DLP} use local energy differences to shape
single- and multi-site flips; multiple-try Metropolis~\cite{Liu2000MTM}
selects among candidate sets; Creutz's demon~\cite{Creutz1983} is the
deterministic microcanonical ancestor of energy-conserving moves.
Isoenergetic cluster moves~\cite{Houdayer2001,Zhu2015ICM} and
equi-energy jumps~\cite{Kou2006EE} are the classical moves closest to
the shell mechanism itself; Sec.~\ref{sec:outlook-test} records why
neither supplies the cheap de-novo shell generator the open question
asks for. These
methods are state of the art for the contests they were built for. Our
zoo result places them relative to the quench on cold rugged targets:
their local information buys energy concentration only at Hamming
distance one, which is precisely the corner of
Fig.~\ref{fig:cfar} the Metropolis filter does not reward.
...
embedded in an exact MH sampler with Gibbs targets; the envelope law
as the negative answer to noise-as-resource for \emph{sampling}; and
the oracle-sufficiency demonstration that shell sampling alone
reproduces the coherent quench's edge --- sharpening the qualitative
large-$d_H$/small-$|\Delta E|$ picture already present in the qe-MCMC
literature~\cite{Ferguson2025,AraiKadowaki2025,Cao2026} --- with the
concentration-at-distance predictor and the classical no-go evidence
of the kernel zoo. The
...
This work was carried out independently, without external funding.
The author developed the theory, experiments, and manuscript with
substantial assistance from Anthropic's Claude models, used under the
author's direction for literature search, derivation checking, code
generation, and drafting. All quantitative claims are backed by the
certification scripts and pre-registered locks distributed with the
manuscript source, and were verified by rerunning them; any remaining
...
confirmation table; the certified R1 and amplitude-damping result
files; and the M01 kernel-zoo table
(\texttt{paper/figures/data/m01\_cfar\_full.csv}, generated by
\texttt{tunnelvision/experiments/D01\_demon/}, which also carries the
demon, multiple-try, discrete-Langevin, and shell-oracle
implementations with their exactness tests, and the M02
isoenergetic-cluster measurements
(\texttt{m02a\_icm\_cfar.csv}, \texttt{m02b\_icm\_gap.csv}) with
the two-replica kernel and its certification tests; the M03 SAT-shell
sampler, its pre-registered gates, and its uniformity and wall-clock
tables live in \texttt{tunnelvision/experiments/M03\_shellhash/}; the
M05 temperature-ladder and pseudo-Boolean-solver comparison in
\texttt{tunnelvision/experiments/M05\_depth/}, whose results doc also
records a state-generator bug that invalidated the depth attribution
of the earlier M03/M04 ``cold'' timings; and the M06 native-solver
scan in \texttt{tunnelvision/experiments/M06\_pbnative/}; the P01
pre-registration, miner race, and results in
\texttt{tunnelvision/experiments/P01\_portal/}; and the sector-support
proof note and its Lindblad certification script in
\texttt{tunnelvision/docs/}). A public archival deposit of both
repositories is available at Zenodo, DOI
\texttt{10.5281/zenodo.22758619}.
